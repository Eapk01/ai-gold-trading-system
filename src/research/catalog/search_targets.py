"""Research search target catalog helpers."""
...
def build_search_target_specs(
    *,
    runtime_target_column: str,
    working_target_spec: object,
) -> List[object]:
    """Return the bounded research search target spec list."""
    try:
...
def describe_search_target(spec_id: str, display_name: str) -> str:
    """Return a plain-language description for a search target."""
    ...
    return SEARCH_TARGET_DESCRIPTIONS.get(spec_id, display_name)


def build_search_target_catalog_rows(
    *,
    target_specs: List[object],
    selected_target_ids: List[str],
) -> List[Dict[str, Any]]:
    """Build GUI-ready catalog rows for search targets."""
        ...
        rows.append(
            {
                ...
                "description": describe_search_target(spec_id, display_name),
